...
describe("f6 input drafts routes", () => {
  ...
  it("fails closed when current lineage has zero matching F2 baselines", async () => {
    const rootDir = testRoot("workbench-server-f6-input-drafts-missing-f2");
    await rm(rootDir, { recursive: true, force: true });
    const server = await buildWorkbenchServer({ rootDir, skipWebAssets: true });
    try {
      const browser = await server.testAuthenticate(SESSION_ID);
      await seedSessionLineage(rootDir, SESSION_ID, { omitF2: true });
      ...
      const response = await server.inject({
        ...
        payload: {
          ...
          proposal: {
            proposalVersion: "f6-analysis-context-proposal-v1",
            userText: "test missing f2 baseline",
            worksheetSelectors: ["Analysis-A"],
            clarifications: [],
          },
        },
...
async function seedSessionLineage(
  rootDir: string,
  sessionId: string,
  options: { duplicateF4?: boolean; duplicateF2?: boolean; omitF2?: boolean } = {},
): Promise<void> {
