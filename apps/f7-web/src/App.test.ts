...
describe("F7 workbench shell", () => {
  ...
  it("9) workflow rail renders three grouped steps and marks step1 current initially", async () => {
    ...
    expect(STYLE_SOURCE).toMatch(/\.workflow-steps\s*\{[^}]*grid-template-columns:\s*repeat\(3,\s*296px\)[^}]*grid-auto-rows:\s*1fr[^}]*justify-content:\s*center/s);
    ...
    expect(STYLE_SOURCE).toMatch(/\.measurement-entry-tabs\s*\{[^}]*flex-direction:\s*column[^}]*align-items:\s*center/s);
