...
export function projectWorkspaceIssue(snapshot: F8SessionSnapshot | undefined, error: TypedError | undefined): WorkspaceIssue | undefined {
  if (error !== undefined) {
    ...
    if (error.code === "evidence_mismatch") return { severity: "warning", title: reasonDisplay(error.code), detail: `Refresh the current ${featureDisplay("F4")} worksheet before continuing.`, action: "refresh" };
