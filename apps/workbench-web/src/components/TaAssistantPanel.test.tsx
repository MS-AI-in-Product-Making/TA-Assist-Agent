...
describe("TaAssistantPanel", () => {
  it("shows English suggested prompts and a concise next-request context summary", async () => {
    ...
    expect(screen.getByRole("textbox", { name: "Ask TA Assist from governed evidence" })).toHaveValue("Compare the baseline and saved Scenario");
  ...
  it("binds worksheet and factor context while suggestions only fill the draft", async () => {
    ...
    expect(screen.getByRole("textbox", { name: "Ask TA Assist from governed evidence" })).toHaveValue("Summarize the current governed evidence");
    expect(onSubmit).not.toHaveBeenCalled();
    fireEvent.click(screen.getByRole("button", { name: "Send message" }));
