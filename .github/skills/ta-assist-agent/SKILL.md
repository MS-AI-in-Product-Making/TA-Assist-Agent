...
**REQUIRED SUB-SKILL: Use design-optimization.** Preserve its workbook entry governance gates without omitting or merging confirmations. For the canonical workspace, use the single-publication sequencing below rather than the legacy W9/W9B two-publication schedule.
...
- For this single-publication workspace flow, finish the governed terminal ADO outcome after F5 and before the single F6 publication, retaining the mode-choice and separate final-write confirmations. Do not use the legacy preliminary-F6 / post-ADO-F6 republication sequence: it would require reopening a completed root or allocating a second root. Final F6 provenance must already contain the validated terminal ADO outcome.
