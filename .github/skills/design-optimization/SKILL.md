...
Each worksheet must include an `optimizationAssessment` list with exactly four governed adjustment classes (`factor_nominal`, `system_mean_shift`, `system_specification`, `factor_tolerance`) and unique priorities. Assessment prose must not fill governed numeric values; governed numbers stay in deterministic calculation claim placeholders only.
