...
- `F6` output: status, root, four artifact paths, final report link, required multimodal mode, decision and validated path/hash, Context/Targets decision outcomes and controlled hashes, candidate/completed/failed option counts, four-state report status (`PASS`, `CONDITIONAL_PASS`, `FAIL`, or `INCOMPLETE`), and blocked worksheet section.

For every successful completion response (workbook mode or existing-artifact mode), render exactly one workspace-relative Markdown link built from the validator-confirmed final report path, in this form: `[Design Optimization Report](test/demo-output/f6-runs/<run-id>/Feature6-Report.md)`. On the next line, render the validator-confirmed complete absolute path in this form: `完整报告路径: C:\<workspace>\test\demo-output\f6-runs\<run-id>\Feature6-Report.md`. Every successful completion must include both the clickable link and the complete absolute report path. Never guess, synthesize, or rewrite either value from directory names; use only the validated final report path, converting that same path to workspace-relative form for the link. If the final report path is missing, not final, outside controlled containment, linked, stale, or validation fails, do not present any report link or complete report path.
...
Resolve the supplied F6 directory or `Feature6-Optimization.json` beneath the controlled publish root. Validate exactly the four-file artifact set: `Feature6-Report.md`, `Feature6-Optimization.json`, `Feature6-Run-Summary.json`, and `manifest.json`. Validate `Feature6-Optimization.json` against `f6OptimizationResultV3Schema`, then verify the manifest artifact map, the required run-summary and manifest input decision ledgers, the interaction language, the two recorded content hashes, workbook/worksheet identities, exact source provenance basename/hash bindings, classifications, blocked worksheet placement, required multimodal identity, evidence/ROI gates, and `reportSummary` consistency.
