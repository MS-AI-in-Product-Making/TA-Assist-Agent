...
For every successful completion response (workbook mode or existing-artifact mode), render exactly two workspace-relative links built from the validator-confirmed report paths, with labels derived from the basename from the validator-confirmed source workbook path with .xlsx removed: `[<Excel basename> - TA Report](test/demo-output/f6-runs/<run-id>/Feature6-Report.md)` and `[<Excel basename> - TA Report PDF](test/demo-output/f6-runs/<run-id>/Feature6-Report.pdf)`. Do not render absolute paths in the success response. Never guess, synthesize, or rewrite any value from directory names; use only the validated final report paths and convert those same paths to workspace-relative form. If either report path is missing, not final, outside controlled containment, linked, stale, or fails hash validation, do not present any report link or successful report result.
