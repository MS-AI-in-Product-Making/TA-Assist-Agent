...
`public` 的本地只读知识库查询；F1 仅接受受控 `confidential` `.xlsx` 字节并创建只读
worksheet catalog，不读取因子表、不计算、不提取图片、不调用外部服务，也不跟踪或导出
原始 `.xlsx`；F2-F7 保持不可用，F4 返回 `feature_not_available`，F8 仅限用于受治理 Skill
运行时验收的匿名 `public` fixture。此阶段不包含真实工程知识、外部 Adapter、模型、ADO、
SharePoint 或 UI 行为。
