...
const ANALYZE_VERB = /(分析|analy[sz](?:e|is))/i;
...
const SESSION_STATUS = /(当前.*(状态|进度|报告|证据)|(?:open|show|view)\s+(?:the\s+)?report|(?:open|show|view|explain).*(?:current|governed).*(?:report|evidence)|status|progress|blocker|阻塞|卡住)/i;
