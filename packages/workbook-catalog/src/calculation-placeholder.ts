...
function createResult(value: unknown): CalculationResult {
  const parsed = calculationResultSchema.safeParse(value);
  if (!parsed.success) throw requestError(REQUEST_SUMMARY);
  return deepFreeze(structuredClone(parsed.data));
}

export function createCalculationPlaceholder(request: unknown): CalculationResult {
