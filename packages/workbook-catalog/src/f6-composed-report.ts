...
function buildV2Worksheet(
  ...
  const sections: F6ComposedEngineeringReportV2["worksheets"][number]["sections"] = {
    ...
    calculationSelfCheck: {
      ...
      rssCheck: consistencyDto(projection.selfChecks.rss),
      rangeChecks: projection.selfChecks.ranges.map(consistencyDto),
      worstCaseCheck: consistencyDto(projection.selfChecks.worstCase),
      worstCaseUpperCheck: consistencyDto(projection.selfChecks.worstCaseUpper),
      worstCaseLowerCheck: consistencyDto(projection.selfChecks.worstCaseLower),
    },
    statisticalResults: { ...v2Section("statistical_results", [baselineEvidenceId]), mean: v2Quantity(calculation.system.mean, unit), adjustedMean: v2Quantity(calculation.system.mean, unit), meanShift: v2Quantity(calculation.system.additionalMeanShift, unit), rssSigma: v2Quantity(calculation.system.rssSigma, unit), ranges: projection.statisticalRanges.map((entry) => ({ sigmaLevel: entry.sigmaLevel, range: { ...entry.range }, formulaCheckId: entry.formulaCheckId })), worstCase: v2Range(projection.margins.worstCase.lowerBound, projection.margins.worstCase.upperBound, unit), formulaChecks: formulaCheckDtos },
