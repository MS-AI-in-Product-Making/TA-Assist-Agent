...
export function readOoxmlWorkbook(bytes: Uint8Array, worksheetNames?: readonly string[], includeImages = true): OoxmlWorkbook {
  try {
    ...
    for (const sheet of onlyChildren(sheetsElement, family.spreadsheetml, "sheet")) {
      ...
      worksheets.set(name, {
        name,
        partName,
        cells: readCells(worksheet, strings, family, cellBudget),
