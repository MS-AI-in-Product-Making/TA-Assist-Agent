  ...
  describe("root F5 data interpretation contracts", () => {
    ...
    describe("F6 composed report v2", () => {
      ...
      const sections = {
        ...
        objectiveAndRequirements: { ...section("objective_and_requirements"), analysisObject: null, analysisCharacteristic: "DIM829, Audio Jack to C bucket Gap", target: quantity(0), lsl: quantity(-0.15), usl: quantity(0.15), targetCpk: 1, requirementIds: [], functionalBoundary: null, passFailCriteria: null },
        operatingConditions: { ...section("operating_conditions", "INSUFFICIENT_EVIDENCE" as const), conditions: [] },
        inputIntegrity: { ...section("input_integrity"), rating: "PARTIALLY_COMPLETE" as const, factors: [{ factor, partName: "Part A", drawingNumber: null, dimId: null, nominal: quantity(0), mean: quantity(0), upperTolerance: quantity(0.2), lowerTolerance: quantity(-0.2), distribution: "normal", sigmaLevel: 4, sigma: quantity(0.05), longTermSafetyFactor: 1, sourceCells: { factorName: "Analysis-A!G14" }, evidenceId: "evidence-baseline", confidence: "HIGH" as const, notes: [] }], findings: [], governanceSummary: { factorCount: 1, drawingNumberMissingCount: 1, dimIdMissingCount: 1, affectedSourceRows: [14] } },
        toleranceLoopDefinition: { ...section("tolerance_loop_definition", "INSUFFICIENT_EVIDENCE" as const), start: null, end: null, responseDirection: null, terms: [], equation: null, reviewRequired: true, loopEvidence: { imageReference: { artifact: "f1", worksheetName: "Analysis-A", relativePath: "images/Analysis-A.png", contentHash: "c".repeat(64) }, toleranceLoopDescription: "DIM829, Audio Jack to C bucket Gap", factorDescriptions: [{ tableId: "table-a", sourceRow: 14, factorDescription: "Factor A" }], visualFacts: [{ statementId: "fact-visual-1", scope: "direction", observedValue: "visible", confidence: "high", reviewStatus: "unreviewed", visibleBasis: "Arrow marker is visible." }], contextSignals: [{ statementId: "signal-context-1", scope: "direction", signalValue: "ambiguous", textBasis: "No governed label mapping yet.", requiresEngineeringReview: true }], requiresEngineeringReview: true, signedEquationAuthorized: false } },
