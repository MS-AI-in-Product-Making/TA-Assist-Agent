...
Agent Skill 触发语“使用F6分析报告”启动 `F0 -> F1 -> F2 -> F3 -> F4 -> F5 -> F6` 受治理 workbook 流程，用户无需手工拼接命令。Skill 保留 two worksheet confirmations：第一次依据 workbook hash 固定 F1/F2 解析范围，第二次只从 F2 ready 且 F1 图片 provenance 有效的 worksheets 中确认下游精确范围。后续按 local-only F3（不发布 ADO）、F4、F5、F6 顺序执行；新 image mode 只创建 immutable `f5-image-observation-v2`。

全部验证完成后，Skill 展示 F1-F6 output ledger，并单独披露 F0 受控版本。每个阶段只有通过 contract、containment、identity、manifest 与 recorded hashes 后才算完成。缺少 supplier 或 datum evidence 时 option 保持 `insufficient_evidence`；缺少受治理 cost coverage 时 ROI 保持 `not_computed`。输入已有 F6 artifact 时只验证并展示六文件 run，不重跑上游。

F6 在 F5 后精确绑定 F2/F3/F4/F5 artifact roots 和至少一个唯一 `--worksheet`。直接 CLI 使用四个顺序固定的 positional roots；app CLI 使用 `--f2-artifacts` 到 `--f5-artifacts`，并固定输出到 `test/demo-output/f6-runs/<F5-root-name>/<UTC-run-id>/`。两者均支持可选 `--supplier-capability`、`--datum-strategy`、`--cost` 和 `--image-observations` evidence。

F5 负责 baseline interpretation/evidence；F6 负责 options、reverse solve、RSS apportionment、feasibility、impact ranking 和 composed report。Top 1 将 baseline 首位贡献因子的 band 收紧 20%；Top 3 将冻结的 baseline 前三项各收紧 30%；两者均保持各自原 band center。Mean centering 始终要求 engineering review。Reverse solve 与 proportional、equal-top-N、capability-bounded、residual-after-centering 四类 RSS policy 都必须经 F4 kernel 回算验证。

Supplier、datum 与 cost 均受 evidence limit。缺失或不匹配时为 `insufficient_evidence`；仅当全部 supported ranked options 都有已知正数治理成本时 ROI 才为 computed，因此 Highest Impact 与 ROI 明确分离。F6 原子发布六个 confidential artifacts：optimization JSON/Markdown、composed report JSON/Markdown、run summary 和 manifest。联合报告包含 workbook summary 与每个 ready worksheet 的十个 sections；F2 blocked worksheet 只进入 workbook Input Validation。源 workbook 只读，F6 不执行 ADO、网络或 workbook write；identity、hash、association、schema、controlled-root、staging、atomic 与 post-commit identity 门禁均 fail closed。
