...
F1 仍是图片唯一物理 owner。新 v2 artifact 在 UUID-scoped 位置单次创建、不可覆盖，并在使用前 readback；F5 loader 对 workbook、worksheet、image 和 artifact hash 保持最终校验权。F5 仅输出 `FACT`、`RULE`、`SIGNAL` 和未排序 `OPTION`，不自动发布 ADO，也不回写 workbook。现有 F5 详细报告及其 F6 delegation 不变；F6 由独立 spec/plan 定义，本次 F5 行为不表示 F6 已实现。
