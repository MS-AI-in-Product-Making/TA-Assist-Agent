...
- direct comment 的 Surface schema 无 body 时，仅当 Surface MCP `update_work_item` 的真实 schema 支持携带完整正文的单个 `add /fields/System.History` patch，F3 才可使用 `System.History` 通道。预览前保存 comment IDs；单次写入后必须恰好一个新增评论，并验证 Work Item ID、正文与 SHA-256 完全一致。通道不可用或校验失败时使用受治理的本地回退；禁止空评论（never empty comment）。
