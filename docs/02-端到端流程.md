...
F1 始终是每个 worksheet 图片的唯一物理 owner。缺少 F1 `imageReference` 或物理图片时，对应 worksheet fail closed。图片及 reference 已验证，但 observation mode 不可用、用户跳过或未生成工件时，F5 继续确定性解读，将图片证据标记为 `not_evaluated` 并给出明确 clarification。任何 observation 都只是可见证据而非图纸真值，必须携带 confidence 并继续受 ME review gate 约束。F5 仅输出 `FACT`、`RULE`、`SIGNAL` 和未排序 `OPTION`，保留每条 F0 规则的版本与适用范围，并记录 assumption，绝不把假设静默当成事实。F5 不自动发布 ADO，也不回写 workbook。
...
flowchart TB
    ...
    M4 -. 未来可选路径 .-> M6["优化（F6 · inactive）<br>F6 当前 unavailable，并返回 feature_not_available<br>当前 run 不进入优化计算，也不保存优化证据"]
    M5 -. 未来可选路径 .-> M6
...
| 优化与版本 | 是否需要调整设计或能力 | 当前 F5 行为仅限于返回 `delegated_to_f6`。调用 F6 时返回 `feature_not_available`；F5 不生成居中、贡献、公差或规格方案，也不提供实时反馈。Future F6 capability 将生成这些方案并提供实时反馈。未来路线图：任何日期/Milestone/版本写入 ADO 的意图仍不在当前 F3 范围。 |
