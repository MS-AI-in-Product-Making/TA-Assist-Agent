...
`available`；F2.1 为严格必填字段校验而标记为 `available`；根 F2、F2.2-F2.4 和 F3-F7 均为
...
| F2.2 | 能力库与分布一致性校验 | `unavailable` | `knowledge-base-v1`, `capability-validation-v1`; `approved-public-knowledge-snapshot` | `capability-validation-request-v1` / `capability-validation-result-v1` | `confidential` | `anonymous-capability-validation-fixture` | `return feature_not_available` |
| F2.3 | 非阻断差异例外处理 | `unavailable` | `exception-resolution-v1`; `approved-exception-policy` | `exception-resolution-request-v1` / `exception-resolution-result-v1` | `confidential` | `anonymous-exception-resolution-fixture` | `return feature_not_available` |
| F2.4 | DIM ID 与 Part Number 质量检查 | `unavailable` | `identifier-quality-check-v1`; `approved-canonical-dim-id-policy` | `identifier-quality-check-request-v1` / `identifier-quality-check-result-v1` | `confidential` | `anonymous-identifier-quality-fixture` | `return feature_not_available` |
