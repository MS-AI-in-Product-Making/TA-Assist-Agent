...
distribution 产生非阻断一致性信号；F2.3 提供受限例外处理，F2.4 提供标识符质量检查。F4 已启用
...
机密输入。Windows Excel Worker 仅用于发布黄金回归，不在生产热路径中。根 F2、F3 和 F5-F7
仍不可用；F0 解读规则和 F4 可用状态均不会启用 F5/F6，
