...
| **F3** | Data-to-drawing link (DIM ID) | Manual, description-based, ambiguous | Cannot link | Uniquely anchors each factor to a drawing dimension by DIM ID (metadata, not image reading); placeholder-first, backfill-later |
...
| **F8** | Closed loop (real Cpk) | None — measured data never returns | None | Feed measured Cpk back by DIM ID → upgrade library from empirical (Tier 3) to measured (Tier 1); gets more accurate the more it is used |
