...
flowchart TB
    ...
    subgraph SEL["2 Worksheet Selection & Confirmation (S1)"]
    ...
    subgraph EXT["3 Parse / Extract Layer (S1)"]
    ...
    subgraph LINK["5 DIM ID Anchor + Active Drawing Loop (S3)"]
    ...
    subgraph MODE["7 Output Modes"]
        M1["Prompt · Data cleansing (S2)<br/>missing-field check + DIM ID completeness<br/>vs Lib 1: tolerance range + distribution"]
        M2["Assign · Method recommendation (S4)<br/>recommend by factor count · both WC / RSS computed"]
        M3["Interpret · Objective 5-section (S6)<br/>FACT/RULE asserted (cite S0) · SIGNAL/OPTION presented<br/>uncertain -&gt; clarification card · judgment left to user"]
        M4["Optimize · What-if / reverse-solve / centering (S7)<br/>mean-shift + contribution economics + RSS apportionment<br/>over-capability -&gt; RED warning"]
    end
    subgraph LOOP["8 Closed Loop (S8)"]
    ...
    subgraph OUT["9 Interaction / Output (S9)"]
