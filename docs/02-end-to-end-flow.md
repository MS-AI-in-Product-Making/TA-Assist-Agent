...
flowchart TD
    ...
    C --> D{"Data cleansing (F2)<br/>missing-field + DIM ID completeness<br/>vs Classified Capability Library"}
    D --> E{"Consistent?"}
    ...
    H1 --> I["Core engine calculation (F5)<br/>both WC and RSS output"]
    H2 --> I
    ...
    CQ --> L
    CC -->|"Evidence sufficient"| L["Objective 5-section interpretation (F6)<br/>each RULE cites its F0 entry"]
    ...
    L --> M6["Tolerance optimization (F7)<br/>centering + contribution economics + RSS apportionment<br/>reverse-solve 2-3 options · over-capability -&gt; RED warning"]
