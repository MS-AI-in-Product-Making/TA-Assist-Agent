...
After F5, the Skill collects optional `f6-analysis-context-v1` and `f6-optimization-targets-v1` artifacts in order. Valid artifacts require separate `Confirm analysis context` and `Confirm optimization targets` calls; neither may be combined with the F3 ADO confirmation. Declined or rejected context continues with explicit gaps. Caller-target scenarios remain unavailable until target confirmation. Independently, `f6-top3-tolerance-policy-v1` runs fixed OP1/OP2/OP3 F4-backed scenarios only for worksheets whose CpkL or CpkU is below the worksheet Target Cpk; passing worksheets without targets remain candidate-only.

After validation, the Skill presents an F1-F6 output ledger plus the controlled F0 version disclosure and Context/Targets decisions. A phase is complete only after contract, containment, identity, manifest, and recorded hashes pass. Missing supplier or datum evidence keeps feasibility at `insufficient_evidence`; missing governed cost coverage keeps ROI at `not_computed`. An existing current F6 artifact entry validates and presents the five-file run without rerunning upstream phases; V1 artifacts are unsupported.
...
F6 owns target-driven options, the single versioned built-in Top-3 policy, reverse solving, RSS apportionment, feasibility, impact ranking, and the final `Feature6-Report.md`; F5 owns baseline interpretation and evidence. Caller-defined quantified scenarios require caller-authorized targets. The only automatic percentages are the fixed `f6-top3-tolerance-policy-v1` OP1/OP2/OP3 matrix, and every result is verified through the F4 kernel; no other automatic percentage scenario is allowed.
