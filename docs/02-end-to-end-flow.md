...
F6 owns target-driven options, reverse solving, RSS apportionment, feasibility, impact ranking, and the final `Feature6-Report.md`; F5 owns baseline interpretation and evidence. Quantified scenarios come only from caller-authorized targets; F6 has no default percentage scenarios. Reverse solves and selected RSS policies are verified through the F4 kernel.
