...
F1 remains the sole physical image owner. New v2 artifacts are immutable, created once in UUID-scoped locations, read back before use, and accepted only after schema/identity checks; the F5 loader remains authoritative for workbook, worksheet, image, and artifact hashes. F5 emits only `FACT`, `RULE`, `SIGNAL`, and unranked `OPTION`, preserves F0 rule version and scope, and neither auto-publishes to ADO nor writes back to the workbook. Its existing detailed report and delegation to F6 are unchanged; F6 is specified separately and is not implemented by this F5 behavior.
