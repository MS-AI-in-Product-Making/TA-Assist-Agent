...
但 Phase 0 不提供自动陈旧锁恢复：发现 `audit.lock` 时，获锁操作在有限重试后以
`dependency_error` 拒绝，绝不自动删除、重命名或修改非本实例创建的锁。释放仅在
锁记录的 owner token 与当前实例匹配时删除该锁。崩溃遗留的 `audit.lock` 属于运行
目录的受控维护事项；维护者必须确认没有运行实例后手动移除锁文件。审计根目录不可
由不受信任主体操作。
...
- 记录包含 `retention_until`。清理先展示有明确作用域的删除计划，获得确认
  后移除相应工件，并写入清理审计事件。
- 导出生成分类 manifest。它拒绝导出 secret；包含机密工件前必须显式确认。持久化
  导出只能在审计未封存时执行：先记录 `export_created`，再原子写入 manifest；封存后
  必须在创建导出目录或文件前 fail closed，不得解除或重建封存。
